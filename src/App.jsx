    ...
    <div ref={containerRef} className="pt-60 pb-80">
      <div className="container mx-auto text-center px-6 relative">
        <div className="reveal mb-32">
          <span className="label-futur mb-12 block animate-pulse">Neural Grid v8.5.0</span>
          <h1 className="h-titan text-[clamp(4.5rem,15vw,18rem)] mb-24 leading-none text-white">
            TITAN <br /> <span className="text-white/10">STREAMS.</span>
          </h1>
        </div>

        {/* Moving Logos Animation (Original Principle) */}
        <div className="reveal relative max-w-5xl mx-auto mb-40 h-[400px] flex items-center justify-center">
          <div className="absolute inset-0 z-0">
            {CHANNELS.map((logo, i) => (
              ...
              >
                <div className="glass-card p-6 rounded-[2rem] border-white/10 group cursor-pointer hover:border-[#00f2ff]/40 transition-all">
                  <img src={logo.url} className="h-10 md:h-14 opacity-40 group-hover:opacity-100 transition-all" alt={logo.name} />
                </div>
              </motion.div>
            ))}
          </div>

          <div className="relative z-10 glass-card p-20 rounded-[5rem] border-[#00f2ff]/20">
            <Tv size={80} className="mx-auto mb-12 text-[#00f2ff] animate-pulse" />
            <h3 className="h-titan text-5xl mb-8">TITAN GRID.</h3>
            <p className="label-futur opacity-40 text-center leading-loose mb-16">Uncompressed 8K RAW Streaming Infrastructure</p>
            {!user ? (
          ...
          <div className="flex gap-40 animate-marquee whitespace-nowrap py-16 px-6">
            {[...CHANNELS, ...CHANNELS].map((logo, i) => (
              <div key={i} className="flex items-center gap-8 group cursor-pointer px-12">
                <img src={logo.url} className="h-10 md:h-14 opacity-20 group-hover:opacity-100 transition-all" alt={logo.name} />
    ...
    <div className="pt-60 pb-80 px-6">
      <div className="container mx-auto">
        <div className="text-center mb-60">
          <span className="label-futur mb-8 block text-[#00f2ff]">Market Protocol</span>
